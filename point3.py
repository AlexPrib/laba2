...
def get_year_from_data(data: list[list[str]], index: int) -> int:
    ...
    Args:
        data: A list with dates and data
        index: The index of the string that points on list in the list
    Returns:
        int: Value of year
    """
    date = data[index][0]
...
def name_for_file(first_part: str, second_part: str) -> str:
    ...
    Args:
        first_part: First part of future file's name
        second_part: Second part of future file's name
    Returns:
        str: Name in special format
    """
    f_p = first_part.replace('-', '') + '_'
    s_p = second_part.replace('-', '') + '.csv'
    return os.path.join('weeks_output', f_p + s_p)
...
def data_to_weeks(file_name: str):
    ...
    if os.path.exists(file_name):
        ...
        if not os.path.exists('weeks_output'):
            os.mkdir('weeks_output')
